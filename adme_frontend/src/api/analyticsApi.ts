...
export const analyticsApi = baseApi.injectEndpoints({
  endpoints: (build) => ({
    platformOverview: build.query<PlatformOverview, void>({
      ...
      providesTags: ["Merchant", "Rickshaw"],
    }),
    advertiserDirectory: build.query<AdvertiserRow[], string | void>({
      query: (approval_status) => ({
        url: "/analytics/advertisers/",
        params: approval_status && approval_status !== "all" ? { approval_status } : undefined,
      }),
      providesTags: ["Merchant"],
    }),
    liveFleet: build.query<FleetRow[], void>({
      query: () => ({ url: "/analytics/live-fleet/" }),
      providesTags: ["Rickshaw"],
    }),
  }),
});
